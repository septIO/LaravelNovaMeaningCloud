...
Nova.booting(function (Vue, router, store) {
    Vue.component('index-laravel-meaningcloud', __webpack_require__(3));
    Vue.component('detail-laravel-meaningcloud', __webpack_require__(6));
    Vue.component('form-laravel-meaningcloud', __webpack_require__(9));
});
...
/* harmony default export */ __webpack_exports__["default"] = ({
    ...
    methods: {
        ...
        getResume: function getResume() {
            ...
            this.loading = true;
            var text = document.querySelector('*[placeholder=' + this.field.relation + ']').innerText;
            text = text.replace(/^ /g, '').replace(/  +/g, ' ').replace(/[\u200B-\u200D\uFEFF\u200E\u200F]/g, '').replace(/\s+/, ' ');

            fetch('https://api.meaningcloud.com/summarization-1.0?key=' + Nova.config.key + '&sentences=' + this.field.sentences + '&txt=' + text).then(function (res) {
                return res.json();
            }).then(function (r) {
                var s = r.summary;
                s = s.replace(/([\.,])([^\n\.\[\]\s])/g, '$1 $2');
                ...
                _this.loading = false;
                _this.recentlyLoaded = true;
                setTimeout(function () {
                    return _this.recentlyLoaded = false;
                }, 2500);
            });
        }
    }
    ...
    [
      _c("template", { slot: "field" }, [
        _c("textarea", {
          ...
          attrs: { id: _vm.field.name, rows: "5", placeholder: _vm.field.name },
        ...
        _c(
          ...
          {
            staticClass:
              "btn btn-default btn-primary inline-flex items-center relative mt-5",
            attrs: { disabled: _vm.loading || _vm.recentlyLoaded },
            on: {
              click: function($event) {
                $event.preventDefault()
                return _vm.getResume($event)
              }
            }
          },
          [
